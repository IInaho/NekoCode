...
NekoCode 是一个基于 Go 的终端 AI 助手，使用 Bubble Tea v2 构建 TUI，支持多 LLM provider（OpenAI / Anthropic / GLM / DeepSeek），具备 Agent 循环、Native Function Calling、工具执行、权限确认、Plan Mode 只读规划、Anthropic Prompt Caching、Auto-review 自动验证、微压缩、Session Memory 和上下文管理机制。
...
│   ├── factory.go                  #   NewClient / Clone — provider 感知的工厂函数
│   └── retry.go                    #   指数退避重试（IsRetryable — HTTP 状态码判定 + Retry）
├── bot/                            # 核心逻辑
│   ├── bot.go                      #   Bot 结构体、依赖注入、公开 API
│   ├── commands.go                  #   内置斜杠命令注册（RegisterDefaults）
...
│   ├── prompt/                     #   分层 system prompt（按 provider 选择）
│   │   ├── builder.go               #     Builder：静态前缀 + 动态段的 system prompt 组装
│   │   ├── sections.go              #     SectionManager：动态段缓存管理
│   │   ├── priority.go              #     PriorityStack：Override/Default 优先级控制
│   │   ├── plan.go                  #     Plan mode prompt 构建
│   │   ├── system_static.md         #     [embed] 静态基础 prompt（身份+工具规则+安全+工程原则）
│   │   ├── system_claude.md         #     [embed] Claude/GPT reasoning：think-first + plan-before-code
│   │   └── system_deepseek.md       #     [embed] DeepSeek/GLM reasoning：act-first + anti-overthinking
...
│   │   ├── report.go               #     ContextReport：上下文诊断报告
...
│   │   ├── truncate.go             #     智能截断（head+tail+错误优先+重复折叠）
...
│   │   ├── guard.go                #     工具输出边界标记（--- BEGIN/END tool_result ---）
│   │   ├── file_cache.go           #     FileStateCache：LRU + mtime 去重，跨子 Agent 共享
│   │   ├── todo.go                 #     TodoFunc 类型、PhaseFunc 类型
│   │   └── builtin/                #     内置工具实现（每工具一文件）
│   │       ├── register.go         #       RegisterAll()：注册全部 11 个内置工具
...
│       ├── retry.go                #     LLM 调用指数退避重试（0.5s→1s→2s，最多4次尝试）
│       └── subagent/               #     子 agent 引擎
...
│           ├── classify.go         #       安全分类器：子 agent 结果审核（pass/warn）
│           └── result.go           #       结构化 Result 类型 + XML 序列化
...
`MicroCompactIfNeeded()` 在每次 `Build()` 前调用，但仅在 token 超过预算 50% 时激活。将旧的 compactable 工具结果内容替换为 `[Old tool result cleared]`。保留数量按 token budget 分级：<32K→5、~64K→8、>=128K→12。大 budget 下保留更多结果，减少 prefix cache 失效。
...
动态内容（todo、summary）放在消息历史之后，确保系统提示词 + 历史消息已缓存的前缀不被破坏。然后应用 20 条消息滑动窗口，按 token 预算从头部修剪。孤儿 tool 消息（无对应 assistant tool_call）被过滤，空内容非 system 消息替换为 `"."`。当 `withTools=true` 时，末尾追加工具选择提醒。
...
`guard.go` 为每个工具结果包裹 `--- BEGIN/END tool_result ---` 边界标记，包含工具名和调用 ID，帮助模型区分工具输出与对话/系统提示。
...
| 机制 | 位置 |
|------|------|
| 关键约束锚定（7 个正则提取 "不要/必须/do not/must" 等，永不压缩） | `ctxmgr/anchor.go` |
...
| `SetThinkingBudget` / `SetReasoningEffort` 跨 provider 互译（不再静默忽略） | `llm/anthropic.go`, `llm/openai_compat.go` |
...
`bot.go` 注册 `SetShouldStop`：当 `searchCount >= 4 && fetchCount == 0` 时强制停止（搜索后从未抓取，模型可能在"想象"搜索结果）。同时 `detectDiminishingReturns`（连续 3 回合 completion token < 200）和 `doomLoopCheck`（4 次连续相同工具调用）触发 `forceSynthesize()`。
...
| Provider | 实现文件 | 要点 |
|----------|---------|------|
| OpenAI / GLM / DeepSeek | `openai_compat.go` | `/chat/completions`，`thinking: {type: "disabled"}` 默认关闭，`reasoning_effort` 控制；`SetThinkingBudget` 自动映射到 `reasoning_effort` |
...
| **内置工具** | `bot/tools/builtin/` | 11 个内置工具的具体实现，通过 RegisterAll() 注册 |
